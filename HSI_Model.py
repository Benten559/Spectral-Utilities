...
class HSI_Model:
    ...
    def set_rgb_by_wv_index(self, r: int, g: int, b: int, norm_max: int = None) -> None:
        ...
        self.rgb = cv2.merge([r, g, b])
        if not norm_max is None:
            self.rgb = self.rgb = cv2.normalize(
    ...
    def set_circular_std_pixel_mask(self, yPixelOffset: int = 500):
        """
        ...
        mask_KeepCalibrationStd = cv2.circle(img=mask_KeepCalibrationStd, center=(
            stdLoc[0], stdLoc[1]+trim_offset), radius=stdLoc[2], color=(255, 255, 255), thickness=-1).astype(np.uint8)
    ...
    def set_mask_by_threshold(self, kernel_center=8):
        ...
        sharp, thresh = cv2.threshold(
            img_gray, 0, 1, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
        self.mask = thresh
        self.sharp = img_gray
...
# Normalize to range
    ...
    def get_all_masked_bands_radiance_array(self):
        ...
        for band in range(len(self.wv)):
            ...
            if self.gain[band] == 0:
                ...
                avg = sum(maskedRadBand[maskedRadBand != 0])/len(maskedRadBand[maskedRadBand != 0])
            radiance.append(avg)
